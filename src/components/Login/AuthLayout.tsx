...
const AuthLayout:FC<{mode?: 'login' | 'signup', customForm?: React.ReactNode, onSubmit?: () => void}> = ({ mode = 'login', customForm, onSubmit }) => {
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 bg-background overflow-hidden">
        ...
        <div className="hidden md:block relative bg-slate-950 overflow-hidden">
          <Slider {...settings} className="h-full auth-slider">
            {images.map((item) => (
              <div key={item.image} className="h-screen w-full relative outline-none">
                <img 
                  src={item.image} 
                  alt="Background" 
                ...
                <div className="absolute bottom-20 left-12 right-12 z-10">
                  <h3 className="text-sm font-medium text-white tracking-widest uppercase mb-2">{item.label}</h3>
                  <h2 className="text-5xl font-bold leading-tight text-white mb-2">{item.title}</h2>
                  <p className="text-lg font-light opacity-80 italic text-white">{item.description}</p>
                </div>
              </div>
            ))}
          </Slider>   
         ...
         <div className="p-8 md:p-0 flex flex-col justify-center w-full mx-auto md:w-[500px] bg-background">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground text-center mt-2 text-lg">
              {isLogin ? 'Glad to see you again! Please log in.' : 'Start your journey with AI-powered insights.'}
            </p>
          </div>
