...
function LightboxModal({ src, onClose }: { src: string; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-md" onClick={onClose}>
            <button onClick={onClose} className="absolute top-6 right-6 z-10 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white text-2xl transition">&times;</button>
    ...
    return (
        <div className="absolute bottom-2.5 left-1/2 -translate-x-1/2 flex gap-1.5 z-20">
            {Array.from({ length: count }).map((_, i) => (
                <div key={i} className="w-1.5 h-1.5 rounded-full transition-all duration-400"
                    style={{ background: i === active ? "var(--brand, #a78bfa)" : "rgba(255,255,255,0.35)", transform: i === active ? "scale(1.4)" : "scale(1)" }} />
            ))}
        </div>
    );
}
    ...
    return (
        <div className="masonry-item mb-3 sm:mb-4 break-inside-avoid">
            <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} {...touchBind}
                ...
                {is4k && <div className="absolute top-2.5 right-2.5 z-20 bg-brand/80 text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider backdrop-blur-sm">4K</div>}
                <div className="absolute inset-0 opacity-30 group-hover:opacity-50 transition-opacity duration-500 pointer-events-none rounded-xl"
            ...
            <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} {...touchBind}
                ...
                {card.testimonial && (
                    <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 z-10">
                        <span className="text-[9px] font-bold tracking-widest uppercase mb-1 block" style={{ color: "var(--brand, #a78bfa)" }}>{card.testimonial.time}</span>
                        <h4 className="text-white font-bold text-sm sm:text-base mb-1 leading-tight">{card.testimonial.title}</h4>
                        <p className="text-gray-200 text-[10px] sm:text-xs italic leading-snug opacity-90 line-clamp-3">"{card.testimonial.text}"</p>
                        {card.testimonial.author && (
                            <div className="mt-2 pt-2 border-t border-white/10 flex items-center gap-2">
                                <div className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-[8px] font-bold" style={{ color: "var(--brand, #a78bfa)" }}>{card.testimonial.author.charAt(0)}</div>
                                <span className="text-white/60 text-[10px] font-medium">{card.testimonial.author}</span>
                            </div>)}
                    </div>)}
            </div>
        </div>
...
// --- Section ---
function TestimonialSection({ section, onImageClick }: { section: Section; onImageClick?: (s: string) => void }) {
        ...
        <div id={section.id} className="mb-14 sm:mb-20 scroll-mt-24">
            <div className="mb-5 sm:mb-7 border-b border-border/50 pb-3">
                <div className="flex items-center gap-2 mb-1">
                    <div className="h-px w-4" style={{ background: "var(--brand, #a78bfa)" }} />
                    <p className="text-[8px] sm:text-[9px] tracking-[0.4em] uppercase font-bold" style={{ color: "var(--brand, #a78bfa)" }}>COLLECTION</p>
                </div>
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-black tracking-tighter text-foreground">{section.title}</h2>
            </div>
            <div className="columns-2 md:columns-3 lg:columns-4 gap-3 sm:gap-4">
                {section.items.filter(c => c.images.length > 0).map((card, i) => {
                    return card.testimonial
                        ? <TestimonialCardComponent key={card.id} card={card} index={i} />
                        : <ImageCard key={card.id} card={card} is4k={is4k} onImageClick={onImageClick} index={i} />;
                })}
            </div>
        </div>
    );
...
// --- Sidebar ---
function Sidebar({ sections, activeId, onNav }: { sections: Section[]; activeId: string; onNav: (id: string) => void }) {
    ...
    return (<>
        <aside className="sticky w-[280px] z-[40] border-r border-border top-[80px] p-8 bottom-0 h-[calc(100vh-80px)] bg-background/50 backdrop-blur-md hidden lg:block">
            <div className="mb-10">
                <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground font-bold mb-1">Discovery</p>
                <h4 className="text-2xl font-bold text-foreground">Our <span className="text-brand-gradient">Showcase</span></h4>
            </div>
            <ul className="space-y-1">
                {sections.map((s) => {
                    const a = activeId === s.id; return (
                        <li key={s.id}><button onClick={() => onNav(s.id)} className="group flex items-center gap-2 w-full text-left py-2 transition-all duration-300">
                            <span className="block h-px shrink-0 transition-all" style={{ width: a ? 32 : 12, backgroundColor: a ? "var(--brand)" : "var(--border)", transitionDuration: "400ms" }} />
                            <span className="text-base transition-all duration-300" style={{ color: a ? "var(--foreground)" : "var(--muted-foreground)", fontWeight: a ? 700 : 500 }}>{s.label}</span>
                        </button></li>
                    );
                })}
            </ul>
        ...
        <nav
            ...
            style={{ transform: mobileNavVisible ? "translateY(0)" : "translateY(-110%)" }}
        >
            <div className="text-center mb-3">
                <p className="text-[9px] uppercase tracking-[0.2em] text-muted-foreground font-bold mb-0.5">Discovery</p>
                <h2 className="font-bold text-foreground">Our <span className="text-brand-gradient">Showcase</span></h2>
            </div>
...
export default function ClientShowcase() {
    ...
    useEffect(() => {
        ...
        SECTIONS.forEach((section) => {
            ...
            if (!el) return;
            const obs = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) setActiveId(section.id); }, { rootMargin: "-40% 0px -50% 0px", threshold: 0 });
            obs.observe(el);
            observers.push(obs);
        });
    ...
    return (
        <div className="min-h-screen flex  flex-col lg:flex-row items-start bg-background text-foreground transition-colors duration-300 font-sans">
            ...
            {lightboxImg && <LightboxModal src={lightboxImg} onClose={() => setLightboxImg(null)} />}
        </div>
    );
}
